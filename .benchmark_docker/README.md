...
## Simulation Gazebo Scans 2D Benchmark

Benchmarks 2D registration methods on synthetic LiDAR scans generated by
the `scan_data_gazebo_sim` Gazebo simulation. Each dataset directory
(`datasets/YYYYMMDD_HHMMSS/`) is one sequence.

Single `--test` flag validates config (5 frames per dataset):
```bash
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_fs2d.sh 4 --test --data-dir /path/to/datasets/
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_sift.sh 4 --test --data-dir /path/to/datasets/
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_kaze.sh 4 --test --data-dir /path/to/datasets/
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_akaze.sh 4 --test --data-dir /path/to/datasets/
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_fourier_mellin.sh 4 --test --data-dir /path/to/datasets/
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_icp.sh 4 --test --data-dir /path/to/datasets/
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_ndt_p2d.sh 4 --test --data-dir /path/to/datasets/
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_loftr.sh 4 --test --data-dir /path/to/datasets/
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_eloftr.sh 4 --test --data-dir /path/to/datasets/
...
are sorted alphabetically and numbered starting at 1):
```bash
# First 5 datasets on machine 1, next 5 on machine 2
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_fs2d.sh 8 --sequences 1-5 --data-dir /path/to/datasets/
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_fs2d.sh 8 --sequences 6-10 --data-dir /path/to/datasets/
```
...
**Machine 1 (CPU-heavy):**
```bash
nohup bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_fs2d.sh 8 --data-dir /path/to/datasets/ > sim_fs2d.log 2>&1 &
nohup bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_sift.sh 8 --data-dir /path/to/datasets/ > sim_sift.log 2>&1 &
```

**Machine 2:**
```bash
nohup bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_kaze.sh 8 --data-dir /path/to/datasets/ > sim_kaze.log 2>&1 &
nohup bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_akaze.sh 8 --data-dir /path/to/datasets/ > sim_akaze.log 2>&1 &
nohup bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_fourier_mellin.sh 8 --data-dir /path/to/datasets/ > sim_fourier.log 2>&1 &
```

**Machine 3:**
```bash
nohup bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_icp.sh 8 --data-dir /path/to/datasets/ > sim_icp.log 2>&1 &
nohup bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_ndt_p2d.sh 8 --data-dir /path/to/datasets/ > sim_ndt.log 2>&1 &
```

**Machine 4 (GPU):**
```bash
nohup bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_loftr.sh 4 --data-dir /path/to/datasets/ > sim_loftr.log 2>&1 &
nohup bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_eloftr.sh 4 --data-dir /path/to/datasets/ > sim_eloftr.log 2>&1 &
nohup bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_lightglue.sh 4 --data-dir /path/to/datasets/ > sim_lightglue.log 2>&1 &
```

### Data format

Datasets are recorded with the `scan_data_gazebo_sim` package:

```bash
# 1. Record a dataset (while sim is running)
ros2 launch scan_data_gazebo_sim record.launch.py
# Drive around, Ctrl+C when done.
# Output will appear at: datasets/YYYYMMDD_HHMMSS/

# 2. Run benchmark
bash .benchmark_docker/simulation_gazebo_scans/run_sim_gazebo_fs2d.sh 4 \
  --data-dir $(pwd)/datasets/
```

The data directory (`/path/to/datasets/`) contains timestamp-named folders,
each holding the required files:
```
datasets/
├── 20260729_152030/           ← one sequence = one recording session
│   ├── metadata.yaml          # scan params (num_beams, angle range, …)
│   ├── poses.csv              # id, time_sec, x, y, yaw
│   ├── transforms.csv         # id0, id1, dx, dy, dtheta
│   └── scans/
│       ├── 000000.npy         # 720 × float32 range array
│       ├── 000001.npy
│       └── …
├── 20260729_153045/
└── …
